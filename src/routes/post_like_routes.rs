...
pub async fn get_post_likes(post_id: Uuid, service: &State<PostLikeService>) -> Result<Json<i64>, (Status, Json<String>)> {
...
pub async fn post_is_liked_by_user(user_id: Uuid, post_id: Uuid, service: &State<PostLikeService>) -> Result<Json<bool>, (Status, Json<String>)> {
...
pub async fn like_post(user_id: Uuid, post_id: Uuid, _jwt: Auth, service: &State<PostLikeService>) -> Result<Json<bool>, (Status, Json<String>)> {
...
pub async fn unlike_post(user_id: Uuid, post_id: Uuid, _jwt: Auth, service: &State<PostLikeService>) -> Result<Json<bool>, (Status, Json<String>)> {
    match service.unlike(user_id, post_id).await {
        Ok(success) => Ok(Json(success)),
        Err(ServiceError::NotFound) => Err((Status::NotFound, Json(format!("Like not found!").into()))),
        Err(e) => {
            eprintln!("Error liking post: {}", e);
            Err((Status::InternalServerError, Json("Internal server error".into())))
        }
