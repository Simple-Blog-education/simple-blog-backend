...
use crate::schema::comments::{id as comments_id, post_id as comments_post_id, user_id as comments_user_id};
...
pub fn get_comments(post_id: Uuid) -> Option<Json<Vec<Comment>>> {
    ...
    let comments_result = comments.limit(50).filter(comments_post_id.eq(post_id)).load(&mut connection).expect("Error loading comments");
...
pub fn get_comments_user(user_id: Uuid) -> Json<Vec<Comment>> {
    let mut connection = PostgresConnection::new();
    let comments_result = comments.limit(50).filter(comments_user_id.eq(user_id)).load(&mut connection).expect("Error loading comments");
...
pub fn put_comment(id: Uuid, comment: Json<CommentChangeset>) -> Option<Json<Comment>> {
    let mut connection = PostgresConnection::new();
    let result = update(comments.find(id)).set(comment.into_inner()).returning(Comment::as_select()).get_result(&mut connection).expect("Error updating comment");
    Some(Json(result))
}
...
pub fn post_comment(comment: Json<NewComment<'_>>) -> Json<String> {
    ...
    let _ = insert_into(comments).values(comment.into_inner())
        .execute(&mut connection).expect("Error saving comment");
