...
pub fn get_all_posts() -> Json<Vec<Post>> {
    ...
    let posts_result = posts.limit(50).select(Post::as_select()).load(&mut connection).expect("Error loading posts");
    Json(posts_result)
}
...
pub fn get_post(id: Uuid) -> Option<Json<Post>> {
    let mut connection = PostgresConnection::new();
    let post_result = posts.find(id).select(Post::as_select()).first(&mut connection).expect("Error loading post");
...
pub fn post_post(post: Json<NewPost<'_>>) -> String {
    let mut connection = PostgresConnection::new();
    let _ = insert_into(posts).values(post.into_inner()).execute(&mut connection).expect("Error saving post");
    "Success".to_string()
}
...
pub fn put_post(id: Uuid, post: Json<PostChangeset>) -> Option<Json<Post>> {
    let mut connection = PostgresConnection::new();
    let updated_post = update(posts.find(id)).set((post.into_inner(), edit_date.eq(now))).returning(Post::as_select()).get_result(&mut connection).expect("Error updating post");
...
pub fn delete_post(id: Uuid) -> Option<String> {
    let mut connection = PostgresConnection::new();
    let _ = delete(posts.find(id)).execute(&mut connection).expect("Error deleting post");
